...
(function() {
    ...
    const ALLOWED_USER_TEXT = "GLOBAL KREASINDO"; // contoh: "ABC"
    ...
    function getLoggedInUserText() {
        ...
        // fallback: NPWP-like number anywhere
        const body = (document.body && document.body.innerText) ? document.body.innerText : "";
        const m = body.match(/\b\d{13,16}\b/);
        if (m) return m[0];
        return null;
    }

    function isUserAllowed() {
        if (!ALLOWED_USER_TEXT || ALLOWED_USER_TEXT.trim() === "") {
            log("User check disabled (ALLOWED_USER_TEXT empty).");
            return true;
        }
        const txt = getLoggedInUserText();
        const ok = txt && txt.includes(ALLOWED_USER_TEXT);
        log("Detected user text:", txt, "Allowed:", ok);
        return ok;
    }
